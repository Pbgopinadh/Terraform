resource "aws_instance" "instance" { # resouce is where we declare the type of instance we want to create and the local name of it.
  count = length(counts) # lenth gets the lenght of the variable dynamically. #3 instances will be created using this function this can be considered as looping mechanism in Terraform.
